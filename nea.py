...
def help_faq():
    print('The web scraping process:'
          "\n\nThe web scraping process may need the user's attention due to the fact that TikTok has anti-bot measures taking place when the program runs.\nAll the user must do is solve the CAPTCHA in order to let the program continue. \nThey are able to pause it as each step"
          " is halted by a user input - what is meant by this is that all the user has to do for each step of the program, is press enter to continue. This is to allow the automation process to run smoothly."
          "\n\n--- --- --- --- ---"
          "\n\nThe network creation process:\n"
          "\nThis will take a while but it mainly runs in the background as there is no visual process to view (should the user wish to not view the statistics indivudally)."
          "\nThe network creation process will create a new file called _-network.gexf which is the network graph that this program calculates its data from."
          "\nUsers are fully allowed to look at the graph using any form of external software, such as Gephi, an open-source network visualisation tool.")
